...
class Function::UbeSkdHelpTest < ActiveSupport::TestCase
  ...
  must "NamedMust is" do
    make_skd
    assert_equal [111, 112, 113, 115, 116],Ubeboard::Skd.named_mult
#Ubeboard::Product.all(:conditions => ["proname in (?)",%w(WF替 PF替 サラン替)]).map(&:id)
#Ubeboard::Skd::NamedMult
  end
  ...
  #    pre_plan_to = skd.ube_plans[0].plan_dry_to
  ...
  ShozoMaintain.each{|ids,ret,runtime_symbols,msg |
    ...
    must "仮割付 #{msg}" do
      ...
      assert_equal [pre_plan_to,pre_plan_to+ret[2].minute,ret[3]], 
      skd.temp_assign_maint(plan,real_ope)
    end
  }
 
  [1].each{
    ids,ret,runtime_symbols,msg =   [[37,36],[1440.0.minute,[115],1440,[115]],
                                     [[(1000-4).hour ,:pf]], 
  ...
  must "作業が重なったときに、併記すべき保守・切り替え" do
    ...
    Ubeboard::Product.where( ["proname in (?)",%w(WF替 PF替 サラン替)]).pluck(:id) #map(&:id)
  end
  ### 抄造の保守仮割付の評価
  Maintain.each{|real_ope,ids,ret,hday,msg |
    next unless real_ope
    must  "#{Ubeboard::Skd::Id2RealName[real_ope]} 前回保全#{hday}日 #{msg} #{ids.join('→ ')}の保守時間" do
      skd=make_skd(ids)
...
#pp plan. ope_length(:shozo)[0]/(1.0.hour)
  ...
  ChangeTimeList.each{|from_pro_id,to_pro_id,change|
    must "ChangeTime #{from_pro_id} -> ,#{to_pro_id}" do
      ...
      pro = Ubeboard::Product.find(from_pro_id)
      from_condition = pro.ope_condition
      line     = pro.shozo
      pro = Ubeboard::Product.find(to_pro_id)
      to_condition = pro.ope_condition
      assert_equal change,
      Ubeboard::ChangeTime.find_by_ope_name_and_ope_from_and_ope_to(line,from_condition,to_condition).change_time
    end
  ...
  ChangeDifferMeigara.each{|real_ope,ids,interval,msg|
    ...
    must "#{Ubeboard::Skd::Id2RealName[real_ope]} #{msg} #{ids.join('→ ')}の切り替え時間" do
      skd = make_skd
      skd.ube_plans=[]
      skd.ube_plans<< Ubeboard::Plan.find(ids[0])
      skd.ube_plans<< Ubeboard::Plan.find(ids[1])
      skd.pre_condition[real_ope]=skd.ube_plans[0]
      plan=skd.ube_plans[1]
      assert_equal( interval,  skd.change_time(plan,real_ope).to_a
                    )
      #pp [Ubeboard::Skd::Id2Ope[Ubeboard::Skd::Real2Ope[real_ope]],skd.named_change_pro_ids(:shozoe,"","16高級")]
    end
  }
