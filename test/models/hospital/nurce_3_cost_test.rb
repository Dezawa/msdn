...
sft_str = :night_total
  must ":night_totalのタイトロールが [10, 3, 9] であるときの看護師の(tight & role_ids).sort" do
    assert_equal [[], [9], [10], [9], [3, 9], [3, 9], [3, 9], [3, 9],
                 ...
                 ], @nurces.map{ |n| tight & n.need_role_ids }
  end
  ...
  must ":night_totalのタイトロールが [10, 3, 9] であるときの看護師のcost" do
    ...
    assert_equal [0,99999,99999, 358, 620, 620,  806,  806, 1152, 1152, 1152, 1498,  2093,2786, 3622, 3622, 4708, 4708, 750],
  ...
  must ":night_totalのタイトロールが [10, 3, 9] であるときの看護師のcost順" do
   ...
   assert_equal [34, 37, 38, 39, 52, 40, 41, 42, 43, 44, 45, 46, 47, 49, 48, 51, 50, 35, 36],
  ...
  must "shift 2のタイトロールが [10, 3, 9] であるときの看護師のcost" do
    ...
    # need_role_ids   [[],[9],[A],[9],[3,9],[3,9],[3,9],[3,9],[A,9],[A,9],[A,9],[A,9],[A],[A,3],[A,3],[A,3],[A,3],[A,3],[A,3]
    assert_equal [0 ,99999,99999,1023,1771,1362 ,2302 ,1362 ,2533 ,2533 ,1948 ,2533 ,3537,4708,2786 ,4708 ,4708 ,4708,1648],
  ...
  must "shift 3のタイトロールが [3, 10, 9] であるときの看護師のcost" do
    ...
    # need_role_ids   [[],[9],[A],[9],[3,9],[3,9],[3,9],[3,9],[A,9],[A,9],[A,9],[A,9],[A],[A,3],[A,3],[A,3],[A,3],[A,3],[A,3]
    assert_equal [0,99999,99999,1023,1362,1771,1362,2302,1948,1948,2533,2533,2093,2786,99999,3622,4708,4708,2143],
