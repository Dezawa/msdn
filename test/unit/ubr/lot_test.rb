...
class Ubr::LotTest < ActiveSupport::TestCase
  ...
  def setup
      ...
      @lotlist = Ubr::LotList.lotlist(true).list
  end

  must "枠数" do
    assert_equal 2091,@Waku.size
  end

  must "ロット数" do
    assert_equal 1626,@lotlist.size
  end
  ...
  must "G123V322--C17---N3 G3097D の枠" do
    lot = findlot_by_lot("G3097D").first
    assert_equal ["1A3D"],lot[1].waku.map(&:name)
    assert_equal ["G3097D"],lot[1].waku.first.lot_list.map(&:lot_no)
  end
