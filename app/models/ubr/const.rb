...
# 0K倉庫
    ...
    x5 = x4 + 2.0         #  |  +-------------------------------+  | y1
                ...
                #
  AC0K = Ubr::SoukoFloor.
    new(:outline => [[x0,y0],[x3,y3]],
        :walls   => [ [[x0,y4],[x0,y0],[x3,y0],[x3,y3],[x5,y3]],
                      [[x4,y3],[x0,y3],[x0,y5]],
                      [[x1,y4],[x1,y1],[x2,y1],[x2,y2],[x5,y2]],
                      [[x4,y2],[x1,y2],[x1,y5]]
                    ]
        #:contents   => %w(0K 0K), # w 69.6m h 47.2m
        #:sufix      => %w(01 14),
        #:max        => %w(13 26),
      #建物基準点から置き場ブロック基準点までの m。ブロックの左上を標準にする
      #:base_points => [ [37.875-1.5-4.0,26.175-3.0],[4.0,26.175-3.0]],
                      
      #:label_pos => [ [1.0,-13],[-1.0,-12]]
      )
  ...
  #### 
    ...
    y0 = 0.0 ; y1 = 28.0
                          ...
                          #  +----    ---------------------    ----+ y1
  AC0L =  Ubr::SoukoFloor.
    new(:outline => [[x0,y0],[x1,y1]],
        :walls   => [ [[x2,y1],[x0,y1],[x0,y0],[x1,y0],[x1,y1],[x5,y1]],
                      [[x3,y1],[x4,y1]]
                    ]
        #:contents   => %w(0L), # w 69.6m h 47.2m
        #:sufix      => %w(01),
        #:max        => %w(13),
      ##建物基準点から置き場ブロック基準点までの m。ブロックの左上を標準にする
      #:base_points => [ [37.875-1.5-4.0,26.175-3.0]],
...
$SoukoFloors = { 
  ...
  Ubr::SoukoFloor.
    new(
        ...
        #                 [56.0,5.0]],
        #:label_pos => [[nil,nil],[20,-0.2],[20,2.4],[8,2.4],             # 枠群の名前記述位置の枠群原点からの相対位置。
        #               [15,-0.2],[15,4],[15,-1.4], [14,1.2], [14,1.2],[-3,-2],[14,-0.2],[nil,nil]],
    ...
    new(:name => "2", 
      ...
      #:label_pos => [[6,1.5],[-10,1.5],[14,-0.2],[14,1.5],[nil,nil],[nil,nil],[nil,nil]],
...
require 'pp'
Ubr::Const
k=$SoukoFloors.keys.sort
k.each{ |souko| f=$SoukoFloors[souko]; f.contents.each_with_index{|c,i| pp [souko,c,f.sufix[i],f.max[i],f.base_points[i][0],f.base_points[i][1],f.label_pos[i][0],f.label_pos[i][1]] }}
