...
# 0K倉庫
  ...
  AC0K = Ubr::Floor.
    new(:name => "0K", :id => 13,
        ...
        :pillars => [ Ubr::Pillar.new(:kazu => [2,2],:start => [0.75,8.6] ,:kankaku => [9.09*4,9.0] ,
                               :size => [1.5,0.4]),
                      Ubr::Pillar.new(:kazu => [3,2],:start => [1.5+9.09,8.6] ,:kankaku => [9.09,9.0] ,
                               :size => [1.5,0.4]),
                      Ubr::Pillar.new(:kazu => [3,1],:start => [0.75+9.09,0.75] ,:kankaku => [9.09,0] ,
                               :size => [0.4,1.5] ),
                      Ubr::Pillar.new(:kazu => [3,2],:start => [2.25+9.09,8.6] ,:kankaku => [9.09,9.0] ,
                               :size => [0.4,3.0] ),
                      Ubr::Pillar.new(:kazu => [4,2],:start => [1.5+4.0,8.6] ,:kankaku => [9.125,9.0] ,
                               :size => [0.4,0.4] ) # (32.875-5.5)/
                    ]
      )

  
  #### 
    x0 = 0.0 ; x1 = 47.5; x2 = x0 + 3.0 ; x3 = x2+2 ; x5 = x1 - 3.0;x4 = x5 -2
    y0 = 0.0 ; y1 = 28.0
                          # x0                                     x1
                          #  +-------------------------------------+ y0
                          #  |                                     |
  ...
  AC0L =  Ubr::Floor.
    new(:name => "0L", :id => 14,
        ...
        :pillars => [ Ubr::Pillar.new(:kazu => [2,8],:start => [5.35+6.1,3.1] ,:kankaku => [6.1*3,3.1] ,
                               :size => [0.6,0.5], :missing =>[[0,7],[0,6]]),
                      Ubr::Pillar.new(:kazu => [7,2],:start => [5.35,3.1+3.1*2] ,:kankaku => [6.1,3.1*3] ,
                               :size => [0.6,0.5] )
                    ]
      )
# 総合倉庫壁位置定数
...
Floors = { 
  "0A" =>   Ubr::Floor.
  new(:name       => "0A",:id => 9,
      :outline    => [[xa-xw,ya-yn],[xe-xw,ys-yn]],
