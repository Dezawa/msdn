...
require 'pp'
#== 計画立案(UbeSkd)
#ロットデータ(Ube::Plan)のインスタンスをhabmする。1月1インスタンスが原則。
#
#3つの機能がある。
#[立案準備]
#     立案対象のロットを Ube::Plan::make_plans_from_lips と Function::boad::boad::SkdHelp#stock で用意する。
...
#
#==== 優先順モード・優先順尊重モード
#割付は基本的には「優先順」(Ube::Plan#jun)の順番で行われる。
#
#優先順モードは必ず優先順で行われるが、尊重モードでは概ね順の通りだが
#Function::Optimize#optimizeが決めた順で行われる。
#
#
class Ube::Skd < ActiveRecord::Base
  #require 'function/ube_skd_help'
  include Ube::Function::SkdResultInput
  include Ube::Function::SkdHelp
  include Ube::Function::SkdFreelist
  include Ube::Function::Optimize
  include Ube::Function::SkdCsv
  include Ube::Function::SkdPdf
  ...
  has_and_belongs_to_many :ube_plans,:class_name => "Ube::Plan"
  ...
  # これは Ube::NamedChange に置くべき性格であるが、Ube::NamedChangeを更新したときに
  ...
  def named_change_pro_id(real_ope,pre_condition_id,post_condition_id)
    ...
    @named_change[[real_ope,pre_condition_id,post_condition_id]] ||=
      Ube::Skd.named_change(real_ope,pre_condition_id,post_condition_id)
  end
  ...
  def self.named_change(real_ope,pre_condition_id,post_condition_id)
    ...
    named_changes = Ube::NamedChange.where( [ sql, ope_name,
    ...
    if named_changes.size > 0
      named_changes.first[:display].split.map{|a00| 
        up = Ube::Product.where(["ope_condition = ? and #{ope} = ?",a00,Id2RealName[real_ope]])
  ...
  MassLimit = 300
  def masslimit
    @masslimit ||=  Ube::Constant.find_by(keyword: "minimum_mass").value rescue MassLimit
  end
  ...
  #-  Ube::Operationで定められる 滞在時間 から、投入間隔＝滞在時間/StayTime
  ...
  #NamedMult = Ube::Product.all(:conditions => ["proname in (?)",%w(WF替 PF替 サラン替)]).map(&:id)
    begin 
      namedMult = Ube::Product.where( ["proname in (?)",%w(WF替 PF替 サラン替)]).pluck(:id)#to_map(&:id)
    rescue
      namedMult = []
    end
  end
  ...
  def unkyu_start
    unless @unkyu_start
      unkyu_start = Ube::Constant.all(:conditions => "name like '運休開始時刻 %'")
      @unkyu_start = 
  ...
  def runshozocount(shozo) ; 
    #logger.info("Ube::Skd:runshozocount(#{shozo}) is nil") unless self["limit_wf_#{shozo}"] 
  ...
  #養生庫の養生庫番号をキーとするハッシュ
  #
  #最初に呼ばれるときに初期化(Ube::Yokojoのインスタンス作成)も行う。
  def yojoko
    unless @yojoko
      @yojoko = Hash.new
      Ube::Yojo::Yojoko.each_key{|no| 
        @yojoko[no]=Ube::Yojo.new(no);@yojoko[no].next_start time_from
      }
    end
    @yojoko
  end
  ...
  def pronamessave
    @pronames ||= Ube::Product.all(:conditions => "hozen = false or hozen is null",
                                 :select => "proname,id"
                                 ).map{ |p| [p.proname,p.id]}
  end

  #Ube::Skdに付いているUbe::Planを完了、仕掛、未実施にわけ、
  ...
  def make_plan(option = {})
    ...
    #debug用 freeListを Ube::Skd.gree_list に保存
  ...
  #については、Function::Ube::Optimize#optimaize を参照されたし。
  ...
  #Ube::Plan の集合を割り付ける
  #1. 養生庫を割り当てる。 Function::Ube::SkdHelp#get_yojoko
  ...
  def delete_hozen_ubeplan_unless_resulted
    ...
    Ube::Plan.update(ids,[attribute]*ids.size)
