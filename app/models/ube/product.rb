...
class Ube::Product < ActiveRecord::Base
...
require 'csv'
  ...
  # DBの変更にダイナミックに追随するために、Ube::ProductControllerのcreate、update, csv_upload
  # のときに再読み込みされる
      ...
      when "酸洗"
        ...
        if [:shozow,:shozoe].include?(real_ope)
          up = self.find_by(proname: hozen,
                                  shozo: Ube::Skd::Id2RealName[real_ope])
        elsif [:dryo,:dryn].include?(real_ope)
          up = self.find_by(proname: hozen,
                                  dryer: Ube::Skd::Id2RealName[real_ope])
        end
  ...
  # 切り替え時間は入っているか。
  # 
  # Ube::Operation.error_cneck での切り替え時間チェックと異なるのは、
  #  Ube::Operation :: 登録されている全品種について調べる
  #  Ube::Product   :: どの製品にも使われていない品種については調べない
  def ope_condition_valid?
    error = []
    # 対応する品種は登録されて居るか
    kind = Ube::Operation.where( "ope_name = '#{ope_condition}'")[0]
    unless kind
  ...
  def check_change(line)
    error = []
      ...
      ch_times = Ube::ChangeTime.where( "ope_name='#{line}' and change_time is not null")
      #   前後の全品種を得る
  ...
  def ope_condition_id
    Ube::Operation.find_by(ope_name: ope_condition).id
  end
end
