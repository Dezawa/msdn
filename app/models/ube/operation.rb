...
#
#======カラム
#ope_name  :: 工程条件名。Ube::Product から参照される
#west  :: 西抄造を使う場合の速度
#east  :: 東抄造を使う場合の速度
#old  :: 原乾燥を使う場合の速度
#new  :: 新乾燥を使う場合の速度
...
class Ube::Operation < ActiveRecord::Base
  ...
  #   今は使われていない。
  # DB変更時にダイナミックに追随させるために、Ube::OpeartionController にて
  # create,update,csv_upload のときに更新される
  # @openames が　nil か、read がtrueのときにDBから読み直される
  def self.names(read=nil)
    if !@openames || read
      @openames ||= Ube::Operation.where( "ope_name not like 'A%'").order("ope_name").
  ...
  def self.hozen_periad  # k = hozen_code == [28, "A02", :shozow]
    @hozen_periad ||= Hash.new{|h,k|
      uo = Ube::Operation.find_by(ope_name: k[1])
      h[k] = uo[Ube::Operation::Real2Field[k[2]]].minute rescue 0
    }
  end
  ...
  # Ube::ChangeTimeも含めたデータの整合性をチェックする。
  # Ube::OperationController#index から呼ばれ、エラーが会ったら表示する。
  # チェック内容
  ...
  def self.error_check
    ...
    [[:west,"西抄造"],[:east,"東抄造"],[:old,"原乾燥"],[:new,"新乾燥"],[:kakou,"加工"]].each{|sym,name|
      ...
      error += Ube::ChangeTime.check(name,kind_list)
    }
    error.uniq
  end

  # Ube::Operation.error_check の下請け。
