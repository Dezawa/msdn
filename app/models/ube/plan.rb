...
class Ube::Plan < ActiveRecord::Base
  ...
  def self.make_plans_from_params(params)
    [params.map{|id,param|  Ube::Plan.new(param)  } ,[]]    
  end
  ...
  def condition
   ...
   # このロットの製造速度(を表すUbe::Operation)を返す。
  def rate
    @rate ||= Ube::Operation.find_by(ope_name: condition)
  end
  ...
  def plans(real_ope,time_sym) 
    ope = Ube::Plan.real2ope(real_ope)
#pp PTime[[ope,time_sym]]
  ...
  Ube::Skd::Ope.each{|ope| %w(plan result).each{|p_r| %w(from to out end).each{|t|
  ...
  def pdf_value(ope = nil)
    unless @pdf_value
      ...
      Ube::Skd::PlanTimesSym.each{|sym| @pdf_value[sym]=(self[sym].strftime("%H:%M") rescue " ----")}
  ...
  #<tt>times</tt> :: 計画の [開始、終了、乾燥のout、乾燥の end]
   def set_plan(real_ope,times)
     ...
     #logger.debug("set_plan: #{lot_no}  #{plan_shozo_from.mdHM} #{plan_shozo_to.mdHM}") #if Ube::Skd::Lot.include?(lot_no)
   ...
   def mass_calculate_by_yojoko_size_or_MassMin(sum)
      ...
      if real_mass <= sum[ube_product_id]
        ...
        mass =  sum[ube_product_id] >= Ube::Skd::MassMin ? sum[ube_product_id] : Ube::Skd::MassMin
   ...
   def current_to
     ...
     logger.info("Ube::Plan#current_to:: #{lot_no} current='#{current}' #{disptime}")
  ...
  # Ube::Skd#make_plan の中で加工を特別扱いしないために、kakou? も定義する
  ...
  #この製品の加工ラインを返す。
  #加工では必要はないが、
  # Ube::Skd#make_plan の中で加工を特別扱いしないために、加工も定義する
  def kakou?   ; :kakou ;end
  def yojo?    ; :yojo  ; end
 ...
 #次工程。割り付けが終了した工程の前の工程
  ...
  def included(from,to)
    #times = Ube::Skd::Ope.map{|ope| 
      #Ube::Skd::PlanTimes[ope].map{|p_f,p_t,r_f,r_t| [self[r_f]||self[p_f] , self[r_t]||self[p_t]]}
    #}.flatten.compact.sort
    #times= plans.each{|sym,ts| p_f,p_t,r_f,r_t=ts; [r_f ||p_f, r_t || p_t] }.flatten.compact.sort
    times= Ube::Skd::Ope.map{|sym| [ plans(sym,:result_from) || plans(sym,:plan_from),
  ...
  def change_time(ope,pre_plan)
    ...
    unless @change_time[[ope,before]]
      ub_change = 
      Ube::ChangeTime.find_by(ope_name: OpeTable[ope],ope_from: before,ope_to: self.condition)
      @change_time[[ope,before]] = 
        if ub_change && ub_change[:change_time]
          ub_change[:change_time].minute 
        else
          unless before.blank?
            err = "Ube::Change: #{OpeTable[ope]},#{before}→#{self.condition} の切替時間が未定義です。ゼロ分とします"
            logger.info(err)
          end
          0
        end
  ...
  #1. Ube::ChangeTime を参照して切り替え時間を得る。
  #2. Ube::Skdの$Meigara に名前があるときは、銘柄違いの補正をする。
  #3. Ube::Skd::ChangeTimeLimit を越える場合は記名切り替え。このときは
  #4. named_change_pro_ids にて Ube::NamedChange を参照し、Ube::Product#id の配列を得る。
  #5. 後処理と前処理の両方が記名の時に発生する。どちらを記載するかは Ube::NamedChangeによる
  #戻り値は [ 切り替え時間, [ Ube::Product#id,Ube::Product#id,Ube::Product#id,,,,]
  #
  #複数の記名切り替えがある時は、長い方の時間で返し、メンテナンスコードは複数返す。
  def change_time_concider_meigara(real_ope,pre_plan,skd)
    return Ube::Function::Maintain.new([0,[nil]]) if real_ope == :yojo || !pre_plan
    if pre_plan.condition == condition and pre_plan.meigara == meigara
      Ube::Function::Maintain.new([Ube::Skd::Round,[]])
    else
      ...
      pro_ids = if change > Ube::Skd::ChangeTimeLimit
      ...
      Ube::Function::Maintain.new([change,pro_ids]) #maintain_code]
    end
  end
  ...
  def unit_time
    unless @unit_time
      @unit_time = Hash.new
      uo = Ube::Operation.find_by(ope_name: condition)
  ...
  #     　　リードタイム：Ube::Skd::DryLead
  #         15分　　　　：Ube::Skd::DryTrail
  #         滞留時間    ：@ope_length[:dry?] （Ube::Operation.find_by_ope_name(ube_product.ope_condition)[:dry?])
  #              単位は分だから、換算すること
  ...
  def ope_length(sym)
    ...
    unless @ope_length 
      ...
      @ope_length[:shozow] =@ope_length[:shozoe] = @ope_length[:shozo] = 
        [unit_time[:shozo] ? (mass.to_f/unit_time[:shozo]).hour.marume(Ube::Skd::Round) : 0 ,0,err]
      err=nil
      err = " #{proname}の乾燥条件が定義されて居ません" if !unit_time[:dry] && condition != "型板"
      logger.info(err) if err
      #length = Ube::Skd::DryLead+(stay*(mass/1000.0+1.0)).marume(Ube::Skd::Round)+Ube::Skd::DryTrail
      if unit_time[:dry]
        ...
        length = ((mass.to_f/unit_time[:dry]).hour + stay).marume(Ube::Skd::Round)
      ...
      @ope_length[:kakou] = 
        [unit_time[:kakou] ? (mass.to_f/unit_time[:kakou]).hour.marume(Ube::Skd::Round) : 0,0,nil]
    end
    @ope_length[sym]
  end
  ...
  def disptime
    Ube::Skd::Ope.map{|ope| "#{ope}:"+Ube::Skd::PlanTimes[ope].map{|t| self[t].mdHM rescue "../..-..:.."}.join("～")}.join("|")
  end

  def start_times
    Ube::Skd::Ope.map{|ope| self[Ube::Skd::PlanTimes[ope][0]].mdHM  rescue "../..-..:.."}.join("  ");
  end
  def if_error(err)
    return unless err
    errors.add(:nil,err)
  ...
  def transfer
   ...
   def hozen?
     @hozen ||= begin
                  ...
                  logger.info("Ube::Plan:No ube_product error:#{id}:#{lot_no} の製品データがおかしい")
                  true
                end
   end
end
module ShozoW
  ...
  #Ube::Plan::Transfer[:yojo] = 60.minute
end
module ThreeByTen
  def transfer#time
