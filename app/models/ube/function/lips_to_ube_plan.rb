# -*- coding: utf-8 -*-
module Ube::Function::LipsToUbePlan
  ...
  LotMin = (LotMax+1)/2
   ...
   # LiPSのデータを元に、Ube::Planのインスタンスを作る。saveはしない。
  # 　LiPSの製品名とUbe::Product#pronameが一致していないとだめ。
  # 　　一致しない時は取り込まず、警告を表示する
  # 　　一応、全角→半角 の正規化は行っている
  # 　製造数量　：LiPSから渡された製造量に (1-不良率)を乗じた数量を製造計画する。
  # 　ロット分割：どの養生庫が使われるか未定なので、標準製造量で割り当てる。
  # 　　　　　　　半端ができたら 最低数量 MassMin のロットにするが、それは Ube::Skd#male_plamn で
  # 　　　　　　　行うので、ここでは半端数をそのまま割り当てておく
  # 　連続lot数 ：同じ製品が連続製造数量が Ube::Product#roundsizeを越えないように分割する
  # 　製造順　　：LiPSの順番。
  # 
  # <tt>lips</tt> :: LiPSのデータ。
  ...
  def make_plans_from_lips(lips)
    ...
    pro.each{|proname,mass|
      ...
      product = Ube::Product.find_by(proname: proname)
      if product.nil?
      ...
      plns =lotNr.to_i.times.map{ 
        Ube::Plan.new(:ube_product_id =>product.id,:mass =>product.lot_size,:lot_no =>"")  
      }
      # 最後のロットは、端数のはず
      ...
      # 多量ロットの分割。1ラウンドの製造数量を Ube::Product#roundsize 以下にする。
      # 未定義の時は警告を出し1万枚にする
      plan = plns.first
    ...
    max_id = Ube::Plan.find(:first,:order => "id DESC").id+1 rescue 1
