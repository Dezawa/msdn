...
module Ube::Function
  ...
  #
  #===割付
  #        休日は assign_holydayを通して 割付とUbe::Plan作成を行う
  #          割付は @FreeListへ  
...
module SkdFreelist
  StartFrom = Ube::Holyday::StartFrom
  #各実工程の空き時間のリスト
  #* 戻り値
  #  空き時間の配列の配列のHash
  ...
  #2. 休日分を削除し(assign_holydayを通し削除と表示用のUbe::Plan作成を行う)
  #3. 休転分を削除し(assign_kyuutenを通し削除と表示用のUbe::Plan作成を行う)
    ...
    unless @FreeList
      ...
      [:dryo,:dryn,:shozow,:shozoe,:yojo,:kakou].each{|ope|
        ...
        when :dryo,:dryn,:kakou
          @FreeList[ope] = 
            Ube::Function::FreeList.new(ope,[time_from,time_to],self) #holydays.dup,maintain)
        when :shozow,:shozoe
          @FreeList[ope] = 
            Ube::Function::FreeListShozo.new(ope,[time_from,time_to],self) #,holydays.dup,maintain)
        when :yojo
          @FreeList[ope] = 
            Ube::Function::FreeListYojo.new(ope,[time_from,time_to],self) #,holydays.dup,maintain)
        end
      }
    end
    @FreeList
  ...
  def assign_maint(real_ope,start,stop,array_hozen_code,option={})
    ...
    #from_to = Ube::Skd::PlanTimes[real_ope]
  ...
  #保守、記名切り替えを表示するために Ube::Plan を作成し、関連付ける
  #real_ope :: 対象となる実工程
  #start    :: 割り当て開始日時
  #stop     :: 割り当て終了日時
  #pro_id   :: 割り当てる保守・記名切り替えの Ube::Product#id
  #jun      :: 順番
  ...
  def beyond_holyday
    @BeyondHolyday ||= (Ube::Skd::BeyondHolyday + 8).hour #starting + ending
  end

  ################################################
  def unkyu(real_ope)
    unless @unkyu
      @unkyu = Hash.new{|h,k| h[k] = Ube::Skd::HolydayUnkyu}

      ucs = Ube::Constant.where('keyword like "%unkyu_start"')
      ucs.each{|uc|
  ...
  #e :: 休日の翌日午前8時
  #
  #始めて呼ばれたときに Ube::Holyday を読んで初期化する
  def holydays
    unless @holydays
      ...
      # 各工程のこの期間の休日予定を得る
      Ube::Holyday.where(["month >= ? and month <= ?",str_from,str_to]).
        each{|holyday|
  ...
  #plan_end :: 休転の終了日時
  #
  #初めて呼ばれたとき、Ube::Maintain を読んで初期化する
  def maintain
    unless @maintain
      @maintain = Hash.new{|h,k| h[k]= []}
      conditions = "plan_time_start <= ? and plan_time_end >= ? "
      maint = Ube::Maintain.find(:all,:conditions=>[conditions ,time_to,time_from])
      #%w(西抄造 東抄造 養生 原乾燥 新乾燥 加工).zip(RealOpe).each{|ope_name,ope|
      maint.each{|m| 
        @maintain[Ube::Skd::RealName2Id[m.ope_name]] << 
        [m.plan_time_start,m.plan_time_end,m.maintain]
      }
      #@maintain.each{|ope_name,v| @maintain[Ube::Skd::RealName2Id[ope_name]] = v }
    end
    @maintain
  end


end # of Ube::SkdFreelist
end # of Function
__END__
