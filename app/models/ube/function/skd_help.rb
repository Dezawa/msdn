...
module Ube::Function
  ...
  #pre_condition :: 各工程最終製品の Ube::Plan
module SkdHelp
  ...
  #4. 製造数量は Ube::Skd#make_plan7 で規格化されている
  #
 ...
 # debug 用出力
  ...
  def set_pre_condition_for_done(plan,real_ope)
    shozo=plan.shozo?
    dry=plan.dry?
    opes = Ube::Skd::Ope.dup; opes[0]=shozo ;opes[2]=dry
    opes = case real_ope
    ...
    opes.each{|ope| 
      return unless times = Ube::Skd::PlanTimes[ope][3]
  ...
  def pro_id_2_lot_no(pro_id)
    ...
    @proid2lotno[pro_id] ||= ( Ube::Product.find(pro_id).ope_condition rescue nil)
  end
  ...
  #  保全情報は ［Ube::Product.id, 製造番号]
  #
  #各工程で以下の保守作業を自動的に割り付ける。
  #* どういう名称にするか、は Ube::Product に登録する
  #* 所要時間は Ube::Operation で定義する。ここにあるのは打ち合わせでの情報
  ...
  def maintain_time(plan,real_ope)
    case real_ope
    when :yojo,:dryo,:dryn ; Ube::Function::Maintain.null #[0,[nil]] 
  ...
  def wf_change(plan,real_ope)
    if plan.proname =~ /^S1/ &&
        running["running_wf_#{real_ope}"]+plan.mass >= Ube::Skd::RunShozoS3Cnt
      Ube::Function::Maintain.hozen_data("WF替",real_ope)
    elsif running["running_wf_#{real_ope}"]+plan.mass >= runshozocount(real_ope)
      Ube::Function::Maintain.hozen_data("WF替",real_ope)
    else
      Ube::Function::Maintain.null#[nil] #[0,[nil]]
    end
  end
  ...
  def pf_change(plan,real_ope)
    if running["running_pf_#{real_ope}"]+
      ...
      Ube::Function::Maintain.hozen_data("PF替",real_ope)
    else
      Ube::Function::Maintain.null # [nil] #[0,[nil]]
    end
  end
  ...
  def maintain_time_kakou(plan,real_ope)
    ...
    if ope_to.hour >=8 && ope_to.wday==3  && # 午前中で水曜
      ...
      Ube::Function::Maintain.hozen_data("予防保全",real_ope)
    else
      Ube::Function::Maintain.null #[0,[nil]]
    end
  end
  #酸洗要否を判断する。
  #- その日はまだ酸洗していない
  ...
  def reset_runtime(real_ope,pro_id,lot_no)
    ...
    when "A08"
      ...
      if running[runtime] <  Ube::Skd::RunShozoS3Cnt
        info += " #{Ube::Skd::Id2RealName[real_ope]} #{pre_plan.lot_no}の後 : #{runshozocount(real_ope)}枚にならないが、S1抄造のため実施する" 
        @message << "WF替: #{Ube::Skd::Id2RealName[real_ope]} #{pre_plan.lot_no}の後 : #{runshozocount(real_ope)}枚にならないが、S1抄造のため実施する" 
      end
  ...
  #切り替え時間の長さと記名切替の Ube::Product.id の配列を返す。
  ...
  def temp_assign_all_plan(plan,shozoDone=nil)
    ...
    if !shozoDone && yojo_plan[0] - shozo_plan[1] > 72.hour #&& (@jun_only & OptimaizeShozo)
      ...
      #酸洗確認しなおす
      sansen_hozen_data = Ube::Function::Maintain.hozen_data("酸洗",plan.shozo?)
  ...
  def temp_assign_shozo(plan)
    ...
    sansen = nil
    sansen_hozen_data = Ube::Function::Maintain.hozen_data("酸洗",real_ope)
  ...
  def temp_assign_dry(plan,force_from_time=nil)
    ...
    if @log
      logger.debug("==== 製造仮割付 #{Ube::Skd::Id2RealName[real_ope]} "+
  ...
  # -  保守候補と切替の遅い方を採る。
  #    　ただし、Ube::NamedChange にて複数の　にある物は併記する
  ...
  def stock
    ...
    skd = Ube::Skd.find(:first,:order => "skd_from desc,id desc",:conditions=>["skd_from < ?",skd_from])
    return [] if skd.nil?

    # 累積の取得
    Ube::Skd::Running.each{|ing,run| self[run]=skd[ing]} 
    ...
    # 各工程最終製品を選ぶ
    dones = Ube::Skd::ResultTimes.map{|r_o,from,to| 
  ...
  def set_jun_for_holiday
    ...
    Ube::Skd::RealOpe.each{|real_ope| 
      plan_from = Ube::Skd::PlanTimes[real_ope][0]
      jun = 0
  ...
  def make_plans_of_holyday_and_maintain
    ...
    holydays.each{|real_ope,hldys| 
      hldys.each{|holyday|
        hc = Ube::Product.holyday_code[holyday[2..3]] || []
        plan = create_hozen_plan(holyday[3],holyday[0],holyday[1],hc[0])
        i += 1
      }
    }
    Ube::Skd::RealOpe.each{|real_ope|
      hc = Ube::Product.holyday_code[["A15",real_ope]]
  ...
  def sumtimes
    %w(_shozo_w _shozo_e _dry_o _dry_n _kakou).each_with_index{|sym,idxs|
      %w(runtime plantime freetime mainttime changetime donetime).each_with_index{|time,idxt|
        self[time+sym] = sumtime(time,Ube::Skd::RealOpe[idxs])
      }
    }
  end
  ...
  #
  #plan_doneを下請けに使って、Ube::Planの予定情報を累計している
  ...
  #=====保守時間累計(mainttime)
  #Ube::Plan#conditionがA02 A03 A06 A07 A08 A09 A11 A13 であるものを
  #保守として累計する。
  #
  #立案時に「実績のない保守・切り替え」は削除してしまっているので、正しい値になっていない。
  def mainttime(real_ope)
    from,to = Ube::Skd::PlanTimes[real_ope][0..1]
    ube_plans.select{|plan| 
  ...
  def set_result_by_plan
    ube_plans.each{|plan| 
      Ube::Skd::Ope.each{|ope|  
        p_f,p_t,r_f,r_t = Ube::Skd::PlanTimes[ope]
