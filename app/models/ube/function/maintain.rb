...
class Ube::Function::Maintain
  ...
  def self.hozen_data(hozen,real_ope=nil)
    ...
    unless @hozen_data[[hozen,real_ope]]
      hozencode = Ube::Product.hozen_code(hozen,real_ope)
      @hozen_data[[hozen,real_ope]] = 
        hozencode ? self.new([Ube::Operation.hozen_periad[hozencode],[hozencode[0]]]) : 
        self.null #[0,[nil],nil]
    end
    @hozen_data[[hozen,real_ope]]
  end
  ...
  def arranged_code
    ...
    if (@hozen_code_list & Ube::Skd.named_mult).size >0
      @hozen_code_list & Ube::Skd.named_mult
    else
      @hozen_code_list
    end
  end
