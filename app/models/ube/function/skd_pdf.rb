...
module Ube::Function
  # 製造指示書を作成する
  # 
  # ロットは色分けする。色は Ube::Product#color で定義する。
  ...
  #             1工程１行で表示される。Ube::Skdのデータを元に割付の全体像を見る。
  ...
  module SkdPdf
    ...
    def skdOutByUbeSkd(title)#,paper="a3",orient="l")
     ...
     # }
      ...
      plans = ube_plans.select{|plan| 
        times = Ube::Skd::PlanTimesSym.map{|sym| plan[sym]}.compact
    ...
    def docOutOpe(title,real_opes,range,items,ope_check)
      ...
      ope_names = real_opes.map{|real_ope| Ube::Skd::Id2RealName[real_ope]}
      ...
      # 予定表出力のマクロ定義 
      makeGraphMacroOpe(real_ope_plan,Ube::Skd::PlanTimes[real_opes.first][0..1],scale,ope_check)
      ### ページ分ループ
      all_days("")
    end
    ...
    def skdAllOutByUbeSkd(title)#,paper="a3",orient="l")
      ...
      @pdf.add( "%%%% /Label %%%%\n").define(:Label){
        #outTitle("月度割付概観 #{skd_from.strftime('%Y/%m/%d')}-#{skd_to.strftime('%Y/%m/%d')}")
        names = Ube::Skd::RealOpe.select{|ope| ope != :yojo}.map{|real_ope| Ube::Skd::Id2RealName[real_ope]}
      ...
      @pdf.add("\n%%%% end Graph%%%%\n").define(:Graph){
        ...
        [:shozow,:shozoe,:dryo,:dryn,:kakou].each_with_index{|real_ope,i|
          @pdf.add("\n%%% #{real_ope} %%%")
          stime,etime = Ube::Skd::PlanTimes[real_ope][0..1]
    ...
    # <tt>plans</tt>;;抄造の時間があるUbe::Planを、西と東にわけ、各々時間順にソートしたもの
    #                 plans = { :shozow => [], :shozoe=>[]
    #                 乾燥、加工の場合も同じ
  ...
  def docOutLotname(real_ope_plans,real_opes,items,yoff)
    x00,y00  =  [Xlabel+2,@Y0-@ObjSkip+yoff] 
    openames = real_opes.map{|real_ope| Ube::Skd::Id2RealName[real_ope]}
    plan_from,plan_to = plan_times = Ube::Skd::PlanTimes[real_opes.first][0..1]
    y0 = y00
    ...
    def docOutLeftYojo (scale,plan_yojoko)#(plans,openames,plan_from,plan_to,scale=1.0)
      ...
      show_labels(%w(養 生 庫),[Xlabel+2,@Y0 - Ube::Yojo::Yojoko.size*@ObjSkip],[0,-24])
      ## 養生庫番号、線
      yojoko_Nos = Ube::Yojo::Yojoko.keys.sort.map{|no| "%3d" % no }
      @pdf.set_font(:point => 10,:font => "/Helvetica")
      @pdf.show_labels( Ube::Yojo::Yojoko.keys.sort.map{|no| "%3d" % no },
                        [xline+2,@Y0- _ObjSkip+yoff],[0,-_ObjSkip])
      @pdf.multi_lines(xline,@Y0- _ObjSkip,@X0+@label_days*Day*scale,@Y0- _ObjSkip,
                       [0,-_ObjSkip],Ube::Yojo::Yojoko.size,0.3)


      x000 = Xlabel+2 + (@Lefts[:ope][1]+@Lefts[:yojoko][1])*@scale
      y0 = @Y0- _ObjSkip+yoff
      #plan_yojoko.each{|no,plansY|
      Ube::Yojo::Yojoko.keys.sort.each{|no| plansY = plan_yojoko[no]
        x0 = x000
    ...
    def makeGraphMacroYojo(plan_yojoko)
      @pdf.define(:Graph) {
        dateline(@sdate,@edate,Ube::Yojo::Yojoko.size*2,OpeScale[:yojo])
    ...
    def plans_run_inthe_range_of_operation(real_opes,ope_check,range)
      plan_from,plan_to = Ube::Skd::PlanTimes[real_opes.first][0..1]
