...
module SkdFreelist
  ...
  def create_hozen_plan(real_ope,start,stop,pro_id,jun=nil)
    ...
    opt = from_to ? { from_to[0] => start,from_to[1]   => stop} : {} 
    lot_no = pro_id_2_lot_no(pro_id)
    plan = Ube::Plan.find_or_create_by(ube_product_id: 0)
  ...
  def ending(real_ope,holy_type)
    unless @ending 
      ...
      ucs = Ube::Constant.where( 'keyword like "%ending"')
  ...
  def starting(real_ope,holy_type)
    unless @starting 
      ...
      ucs = Ube::Constant.where( 'keyword like "%starting"')
  ...
  def maintain
    unless @maintain
      ...
      maint = Ube::Maintain.where([conditions ,time_to,time_from]).to_a
