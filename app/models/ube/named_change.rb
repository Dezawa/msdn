...
#   display          :: 切り替えの製造番号　A??
#
class Ube::NamedChange < ActiveRecord::Base
  extend CsvIo
  self.table_name = 'ube_named_changes'

  belongs_to :pre_condition,:class_name => "Ube::Operation",:foreign_key => :pre_condition_id
  belongs_to :post_condition,:class_name => "Ube::Operation",:foreign_key => :post_condition_id
