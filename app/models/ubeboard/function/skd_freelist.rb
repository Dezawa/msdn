...
module Function
  ...
  #
  #===割付
  #        休日は assign_holydayを通して 割付とUbePlan作成を行う
  #          割付は @FreeListへ  
...
module UbeSkdFreelist
  ...
  #2. 休日分を削除し(assign_holydayを通し削除と表示用のUbePlan作成を行う)
  #3. 休転分を削除し(assign_kyuutenを通し削除と表示用のUbePlan作成を行う)
  ...
  #保守、記名切り替えを表示するために UbePlan を作成し、関連付ける
  #real_ope :: 対象となる実工程
  #start    :: 割り当て開始日時
  #stop     :: 割り当て終了日時
  ...
  def beyond_holyday
    @BeyondHolyday ||= (UbeSkd::BeyondHolyday + 8).hour #starting + ending
  end
  ...
  #e :: 休日の翌日午前8時
  #
  #始めて呼ばれたときに UbeHolyday を読んで初期化する
  def holydays
    unless @holydays
      ...
      [:shozow,:shozoe,:dryo,:dryn,:kakou].each{|ope|
        days = holydays.inject([]){|hd,days| hd += days[ope]}
        days.each_with_index{|h,d| 
          day1 = day0 + d.day
          @holydays[ope] << [day1,day1+1.day                       ,1,ope] if h == "1" #休日
          @holydays[ope] << [day1+UbeSkd::HolydayShukkin,day1+1.day,1,ope] if h == "2" #休出
          @holydays[ope] << [day1+UbeSkd::HolydayKarou,day1+1.day  ,1,ope] if h == "3" #過労
          @holydays[ope] << [day1+unkyu(ope),day1+1.day            ,4,ope] if h == "4" #運休
        }
      }
    end
    @holydays
  ...
  #plan_end :: 休転の終了日時
  #
  #初めて呼ばれたとき、UbeMaintain を読んで初期化する
  def maintain
    unless @maintain
      ...
      #@maintain.each{|ope_name,v| @maintain[UbeSkd::RealName2Id[ope_name]] = v }
    end
    @maintain
  end


end # of UbeSkdFreelist
end # of Function
__END__
