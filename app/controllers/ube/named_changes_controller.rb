# -*- coding: utf-8 -*-
class Ube::NamedChangesController < CommonController #ApplicationController
  include Actions
  ...
  Labels = 
    [# colmn,ラベル,help,type,size or choice
     ...
     HtmlSelect.new(:pre_condition_id , "前品種" ,:include_blank=>true,
                    :correction => Proc.new{Ube::Operation.names},:display =>:pre_con_name),
     HtmlSelect.new(:post_condition_id, "後品種" ,:include_blank=>true,
                    :correction => Proc.new{Ube::Operation.names},:display =>:post_con_name),
     HtmlSelect.new(:ope_name      , "工程名" , :correction =>Ube::Skd::OpeName),
     HtmlText.new(:display       , "表示"   ,:size=>4)
    ]

  def set_instanse_variable
    @Model= Ube::NamedChange
    @TYTLE = "記名切り替え"
    @labels=Labels
