# -*- coding: utf-8 -*-
class Ube::ChangeTimesController < CommonController #ApplicationController
  include Actions
  ...
  def set_instanse_variable
    @Model  = Ube::ChangeTime
  end

  def index
    @TYTLE = "切り替え時間一覧"
    @errorse=Ube::Operation.error_check.join("<br>")
    @ope = Ope
  ...
  def csv_upload
    if params[:csvfile].blank?
      ...
      redirect_to  :controller => :ube,:action=>:top
      return
  ...
  def csv_out
    csvdata = NKF.nkf("-s",Ube::ChangeTime.csv_out)
