...
require 'tempfile'
#
class Ube::SkdController < CommonController #ApplicationController
  include Actions
  ...
  # LiPS CSVファイルを読み込み、Ube::Planを作る
  # LiPS_load_subにて表示する
  def lips_load
   @plans,@error = Ube::Plan.make_plans_from_lips(params[:csvfile])
  ...
  def lips_load_sub(plans=nil)
    ...
    Ube::Plan.delete_all("ube_product_id is null")
  ...
  def csv_out
    ...
    begin 
      ...
      @model = Ube::Skd.find(id)#,:include =>  {:ube_plans => :ube_product})
  ...
  def csv_download  #:nodoc:
    @model = Ube::Skd.find( params[:id])# ,:include => {:ube_plans => :ube_product})
    csv = @model.csvout
  ...
  def index  #:nodoc:
    @labels = Labels
    @models = Ube::Skd.all#(:include => :ube_plans )#=> :ube_product})
    #render :action => "test"
  end
  ...
  def edit_sub(params)  #:nodoc:
    ...
    @plabels= PlanLabelsE
    @EditOnly = EditOnly
    @model = Ube::Skd.find( params[:id])#,:include => {:ube_plans => :ube_product})[0]
    #@model  = Ube::Skd.where(id: 1).joins(:ube_plans)#,:include => {:ube_plans => :ube_product})[0]
    ...
    #@sortkey = (params[:sort] || 'jun' )
    plan_sort(@sortkey)
    #logger.debug("Ube::SkdCont#edit_sub: @sortkey=#{@sortkey}")
    render :action => :edit
  end

  def show_sub(params)  #:nodoc:
    @model = Ube::Skd.find(params[:id])#,:include => :ube_plans)[0]
    @sortkey = (params[:sort] || 'jun' )
  ...
  def create
    ...
    [:skd_from,:skd_to].each{|skd_time| 
      if (timestr = params[:ube_skd][skd_time]).present?
    ...
    if ! params[:excelfile].blank?
      ...
      plans,errors = Ube::Plan.make_plans_from_lips(params[:csvfile])
    elsif params[:ube_plan] #&& params[:ube_plan].size>0
      plans,errors = Ube::Plan.make_plans_from_params(params[:ube_plan])
    else
  ...
  def makeplan
    ...
    @RunTimeLabels=RunTimeLabels
    @plabels= PlanLabelsE
    @model = Ube::Skd.find(params[:id])#,:include => :ube_plans)#{:ube_plans => :ube_product})
  ...
  def update
    ...
    plans.each{|id,plan| 
      next unless ube_plan = Ube::Plan.find(id) rescue nil
      ...
      Ube::Skd::Reuslts.each{|time| next if plan[time].blank? 
    ...
    @model = Ube::Skd.find(params[:id])#,:include => :ube_plans)
    ...
    @sortkey = (params[:sort] || 'jun' )
    plan_sort(@sortkey)
    #logger.debug("Ube::SkdCont#update_false: @sortkey=#{@sortkey}")
      render :action=>:edit
    end
  ...
  def input_result
    @id = params[:id]
    @model = Ube::Skd.find(@id,:select => "id,skd_from,skd_to")
  end

  def update_result
    @model = Ube::Skd.find(params[:ube_skd][:id])
...
#class Ube::SkdController < ApplicationCon<troller
