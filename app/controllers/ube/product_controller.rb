# -*- coding: utf-8 -*-
class Ube::ProductController < CommonController #ApplicationController
  include Actions
...
require 'csv'
  ...
  def destroy
    if Ube::Plan.find_by(ube_product_id: params[:id])
      flash[:message] = "この製品は使われているので削除できません"
      redirect_to :action => :index 
    else
      @product = Ube::Product.find(params[:id])
      @product.destroy
      
      Ube::Product.products true
